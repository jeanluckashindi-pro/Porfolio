        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-card-2 to-card-1 py-16 sm:py-24">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-text mb-6">Publications & Articles</h1>
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {publications.filter(p => p.featured).map((article, index) => (
              <div key={index} className="mb-12">
                <div className="bg-gradient-to-br from-card-2 to-card-1 border border-darkGray rounded-xl overflow-hidden hover:border-primary/30 transition-all">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8 sm:p-12">
                    <div className="lg:col-span-2">
                      ...
                      <h2 className="text-2xl sm:text-3xl font-bold text-text mb-4 hover:text-primary transition-colors cursor-pointer">
                        {article.title}
                      </h2>
                      ...
                      <div className="flex flex-wrap gap-2 mb-6">
                        {article.tags.map((tag, i) => (
                          <span key={i} className="px-3 py-1 bg-darkBlue text-text/60 text-xs rounded-[5px]">{tag}</span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center justify-center">
                      <div className="w-full h-48 lg:h-full bg-gradient-to-br from-card-2 to-card-2 rounded-xl flex items-center justify-center hover:scale-105 transition-transform cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {publications.filter(p => !p.featured).map((article, index) => (
                <div key={index} className="group bg-card-2 border border-darkGray rounded-xl p-6 hover:border-primary/30 transition-all hover:-translate-y-1 cursor-pointer">
                  ...
                  <h3 className="text-lg font-bold text-text mb-3 group-hover:text-primary transition-colors">
                    {article.title}
                  </h3>
                  
                  <p className="text-sm text-text/60 mb-4 leading-relaxed line-clamp-3">
                    {article.excerpt}
                  </p>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    {article.tags.slice(0, 2).map((tag, i) => (
                      <span key={i} className="px-2 py-1 bg-darkBlue text-text/50 text-xs rounded-[5px]">{tag}</span>
                    ))}
                  </div>
                  
                  <div className="flex items-center justify-between text-xs pt-4 border-t border-darkGray">
            ...
            {/* Newsletter CTA */}
            <div className="mt-16 bg-gradient-to-br from-card-2 to-card-2 border border-primary/30 rounded-xl p-8 sm:p-12 text-center">
              <div className="max-w-2xl mx-auto">
                <div className="w-16 h-16 bg-darkBlue rounded-full flex items-center justify-center mx-auto mb-6">
                  <BookOpen className="w-8 h-8 text-primary" />
                </div>
                <h3 className="text-2xl sm:text-3xl font-bold text-text mb-4">Restez informé</h3>
                ...
                <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-darkBlue border border-darkGray rounded-lg text-text placeholder-text/40 focus:outline-none focus:border-primary/50 transition-colors"
                  />
                  <button className="inline-flex items-center justify-center gap-2 bg-primary text-background px-6 py-3 rounded-lg font-semibold hover:bg-darkBlue transition-all whitespace-nowrap">
                    S'abonner
                  </button>
                </div>
